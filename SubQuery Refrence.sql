...
-- by retrieving all the data from the 'projectdb' database

-- 1.1: Retrieve all records from the employees table
SELECT * FROM employees;

-- Retrieve all records from the departments table
SELECT * FROM departments;

-- Retrieve all records from the regions table
SELECT * FROM regions;
...
-- 2.1: Retrieve employee_id, first_name, hire_date, and department
-- of all employees, ordered by the hire date using ROW_NUMBER()
SELECT employee_id, first_name, department, hire_date,
ROW_NUMBER() OVER (ORDER BY hire_date) AS Row_N
FROM employees;

-- 2.2: Retrieve employee_id, first_name, and hire_date
-- of employees for each department, ordered by hire_date
-- The ROW_NUMBER() is partitioned by department
SELECT employee_id, first_name, hire_date,
ROW_NUMBER() OVER (PARTITION BY department ORDER BY hire_date) AS DepartmentRow
FROM employees;
...
-- 3.1: Using ROW_NUMBER() to assign a ranking to employees
-- within each department based on salary, ordered in descending order
SELECT first_name, email, department, salary,
ROW_NUMBER() OVER (PARTITION BY department
                  ORDER BY salary DESC) AS DepartmentRanking
FROM employees;

-- 3.2: Using RANK() to assign a rank to employees
-- based on their salary, ordered in descending order
-- RANK() will assign the same rank to employees with the same salary
SELECT first_name, email, department, salary,
...
-- Exercise 3.1: Retrieve details of employees hired on or before 31st Dec, 2005
-- and who are in the 'First Aid', 'Movies', and 'Computers' departments.
-- The result will show their first name, email, department, salary, hire date,
-- and rank them within their department based on salary in descending order.
...
-- The result will show the department name and the number of employees in that department,
...
-- 3.3: Retrieve the fifth-ranked salary for each department.
-- This uses a subquery to assign ranks to employees within each department
-- based on salary in descending order, and only returns those ranked 5th.
SELECT * 
FROM (
    SELECT first_name, email, department, salary,
...
-- Create a common table expression (CTE) to retrieve the customer_id
-- and the number of purchases each customer has made from the mall.
-- The result will be ordered by the number of purchases in descending order.
...
-- 3.4: Understand the difference between ROW_NUMBER, RANK, and DENSE_RANK.
-- This query assigns row numbers, ranks, and dense ranks to customers based on their number of purchases.
-- ROW_NUMBER assigns unique row numbers without skipping, RANK skips numbers when there are ties,
-- and DENSE_RANK does not skip numbers when there are ties.
WITH purchase_count AS (
    SELECT customer_id, COUNT(sales) AS purchase
    FROM sales
    GROUP BY customer_id
...
-- In this task, we will learn how to break/page
-- the result set into groups using NTILE()
----------------------------

-- 4.1: Group the employees into five groups based on salary in descending order.
-- NTILE(5) will divide the employees into five roughly equal groups.
SELECT *, 
NTILE(5) OVER(ORDER BY salary DESC) AS GroupNumber
FROM employees;

-- Group the employees into three different sets of groups:
-- NTILE(4) divides employees into 4 groups, NTILE(10) into 10 groups, and NTILE(100) into 100 groups.
SELECT *, 
...
-- 4.2: Group the employees into five groups within each department
-- based on salary in descending order. Each department will have five groups.
SELECT first_name, email, department, salary,
NTILE(5) OVER(PARTITION BY department ORDER BY salary DESC) AS GroupNumber
FROM employees;

-- Create a CTE (Common Table Expression) to return employee details,
-- and group the employees into five groups based on their salaries.
WITH salary_ranks AS (
SELECT first_name, email, department, salary,
NTILE(5) OVER(ORDER BY salary DESC) AS rank_of_salary
FROM employees)

-- 4.3: Find the average salary for each group of employees based on their salary rank.
-- This groups employees by their salary rank and calculates the average salary for each group.
...
-- 5.1: Retrieve the number of employees in each department.
-- The result will show the department name and the employee count,
...
-- 5.2: Retrieve the first names, department, and number of employees working in that department.
-- This uses a subquery to get the department count for each employee.
...
-- Alternative solution: Retrieve first names, department, and department count using an aggregate window function.
-- This avoids the need for a subquery and counts the employees within each department.
SELECT first_name, department,
COUNT(*) OVER(PARTITION BY department ORDER BY department ASC) AS dept_count
FROM employees;

-- 5.3: Calculate the total salary of all employees.
-- This uses SUM() as a window function to calculate the running total of salaries ordered by hire date.
SELECT first_name, department, hire_date,
SUM(Salary) OVER(ORDER BY hire_date) AS TOTAL_SALARY
FROM employees;

-- 5.4: Calculate the total salary for each department.
-- SUM() is used as a window function to calculate the running total of salaries within each department,
...
-- Exercise 5.1: Calculate the running total of salary for each department,
-- ordered by hire date. The running total is stored in the 'running_total' column.
...
-- In this task, we will continue learning how to use
-- aggregate window functions in SQL to analyze data across partitions
------------

-- Retrieve distinct region IDs from the employees table.
-- This helps us identify all unique region IDs in the dataset.
SELECT DISTINCT region_id
FROM employees;

-- 6.1: Retrieve the first name, department, and region of employees,
-- along with the count of employees in each department and each region.
-- COUNT() is used as a window function to calculate these counts.
SELECT first_name, department,
COUNT(*) OVER(PARTITION BY department) AS dept_count, -- Total employees per department
region_id,
COUNT(*) OVER(PARTITION BY region_id) AS region_count -- Total employees per region
FROM employees;

-- Exercise 6.1: Retrieve the first names, department, and region ID of employees
-- in region 2, along with the count of employees in each department.
-- This uses the same window function logic but filters results for region 2.
SELECT first_name, department, region_id, 
COUNT(*) OVER(PARTITION BY department) AS dept_count -- Total employees per department
FROM employees
WHERE region_id = 2;

-- Create a common table expression (CTE) to retrieve customer_id, ship_mode,
-- and how many purchases each customer has made in the mall, grouped by customer and ship mode.
WITH purchase_count AS (
    SELECT customer_id, ship_mode, COUNT(sales) AS purchase -- Count of purchases per customer and ship mode
    FROM sales
    GROUP BY customer_id, ship_mode
)

-- Exercise 6.2: Calculate the cumulative sum of customers' purchases
-- for different shipping modes. This uses SUM() as a window function to get the running total.
SELECT customer_id, ship_mode, purchase, 
SUM(purchase) OVER(PARTITION BY ship_mode ORDER BY customer_id ASC) AS sum_of_sales -- Cumulative total per ship mode
FROM purchase_count;


-------------
-- Task Seven: Window Frames - Part One
-- In this task, we will learn how to use window frames 
-- to define a subset of rows in the result set for window functions.
------------------

-- 7.1: Calculate the running total of salary for all employees.
-- This retrieves the first name, hire date, and salary of all employees
-- ordered by hire date to get the total salary up to the current row.
SELECT first_name, hire_date, salary
FROM employees
ORDER BY hire_date;

-- The solution: Calculate the running total using SUM() with window frames.
-- The frame is defined as all rows from the beginning (UNBOUNDED PRECEDING) 
...
-- 7.2: Add the current row's salary and the previous two rows' salaries.
-- This uses ROWS BETWEEN 2 PRECEDING AND CURRENT ROW to define the frame.
SELECT first_name, hire_date, Salary, 
SUM(Salary) OVER(ORDER BY hire_date
				 ROWS BETWEEN 2 PRECEDING AND CURRENT ROW) AS running_total
FROM employees;

-- 7.3: Calculate the running average of salary using a window of the current row
-- and the previous two rows. This provides an average of salaries within the frame.
SELECT first_name, hire_date, Salary, 
AVG(Salary) OVER(ORDER BY hire_date
				 ROWS BETWEEN 2 PRECEDING AND CURRENT ROW) AS running_AVG
FROM employees;

-- Challenge: What will the result of this query be?
-- This query sums the salary of the current row and the previous three rows,
-- creating a running total for the specified window frame.
SELECT first_name, hire_date, salary,
SUM(salary) OVER(ORDER BY hire_date 
				 ROWS BETWEEN 3 PRECEDING AND CURRENT ROW) AS running_total
FROM employees;


------------------------------
-- Task Eight: Window Frames - Part Two
-- In this task, we will further explore how to order
-- and manage data within window frames using SQL functions.
--------------------------

-- 8.1: Review of the FIRST_VALUE() function.
-- This retrieves the first value in the department column 
-- when the departments are ordered alphabetically.
...
-- This query retrieves the first and last department in the table 
-- when ordered alphabetically using window functions. 
-- LAST_VALUE() is used with a RANGE clause to specify the window frame 
...
-- Create a common table expression (CTE) to retrieve customer_id
-- and the total number of purchases for each customer from the sales table.
WITH purchase_count AS (
    SELECT customer_id, COUNT(sales) AS purchase -- Total purchases per customer
    FROM sales
    GROUP BY customer_id
)

-- What do you think this will return?
-- This query calculates the maximum number of purchases for each customer
-- and also retrieves the maximum purchase for the current row and the next row.
-- MAX() as a window function is used with the ROWS clause to specify the frame.
SELECT customer_id, purchase, 
MAX(purchase) OVER(ORDER BY customer_id ASC) AS max_of_sales, -- Max purchases up to the current row
MAX(purchase) OVER(ORDER BY customer_id ASC
				  ROWS BETWEEN CURRENT ROW AND 1 FOLLOWING) AS next_max_of_sales -- Max of current and next row
FROM purchase_count;


-----------------
-- Task Nine: GROUPING SETS, ROLLUP() & CUBE()
-- In this task, we will learn how GROUPING SETS, ROLLUP, and CUBE
-- work to generate subtotals and totals across multiple levels of aggregation.
----------------------

-- 9.1: Find the sum of the quantity for different shipping modes.
-- This basic GROUP BY query aggregates total quantities for each ship mode.
SELECT ship_mode, SUM(quantity) 
FROM sales
GROUP BY ship_mode;

-- 9.2: Find the sum of the quantity for different product categories.
-- This query aggregates total quantities for each category in the sales table.
SELECT category, SUM(quantity) 
FROM sales
GROUP BY category;

-- 9.3: Find the sum of the quantity for different subcategories.
-- This query aggregates total quantities for each subcategory in the sales table.
SELECT sub_category, SUM(quantity) 
FROM sales
GROUP BY sub_category;

-- 9.4: Use the GROUPING SETS clause.
-- GROUPING SETS allows you to define specific groupings for aggregation.
-- This query calculates the sum of quantities for each ship mode, category, and subcategory.
...
-- ROLLUP creates subtotals for each combination of ship mode, category, and subcategory,
-- including the grand total at the end.
SELECT ship_mode, category, sub_category, SUM(quantity)
FROM sales
GROUP BY ROLLUP (ship_mode, category, sub_category);

-- 9.6: Use the CUBE clause.
-- CUBE generates subtotals for all possible combinations of ship mode, category, and subcategory.
-- It includes totals for each level and the grand total.
SELECT ship_mode, category, sub_category, SUM(quantity)
FROM sales
